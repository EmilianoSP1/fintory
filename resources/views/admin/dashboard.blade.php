...
<div x-show="transfer_bruto > 0" class="text-sm text-gray-600 mt-1 dark:text-gray-300">
    Comisión por transferencia (0.36%): 
    <strong x-text="(transfer_bruto*0.0036).toFixed(2)"></strong><br>
    Total neto ingreso: 
    <strong x-text="(transfer_bruto > 0 ? (transfer_bruto-(transfer_bruto*0.0036)).toFixed(2) : '0.00')"></strong>
</div>
...
{{-- ==================== TOTALES Y CONCEPTOS INGRESOS/EGRESOS ==================== --}}
    ...
    <div class="flex flex-row gap-5 items-center">
        ...
        <div class="flex flex-row gap-2">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 text-center min-w-[95px]">
                ...
                <span class="block text-base font-bold text-gray-800">${{ number_format($totalEfectivo, 2) }}</span>
            </div>
            <div class="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 text-center min-w-[95px]">
                <span class="block text-[10px] text-gray-600 uppercase font-semibold mb-1">TARJETA</span>
                <span class="block text-base font-bold text-gray-800">${{ number_format($totalTarjeta, 2) }}</span>
            </div>
            <div class="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 text-center min-w-[95px]">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 text-center min-w-[95px]">
                ...
                <span class="block text-base font-bold text-gray-800">${{ number_format($totalTransferencia, 2) }}</span>
            </div>
        </div>
    </div>
...
{{-- Scroll superior minimalista (invisible pero funcional) --}}
<div id="scrollbar-superior" 
     style="width: 100%; overflow-x: auto; overflow-y: hidden; height: 12px; margin-bottom: 1px;">
...
@php
    ...
    if($e && $e->egreso_tipo === 'Crédito') {
        if($e->pagado) {
            $trClass = 'bg-green-100'; // Ya pagado
        } else {
